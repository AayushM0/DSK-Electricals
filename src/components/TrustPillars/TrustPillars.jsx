            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {TRUST_PILLARS.map(({ id, number, title, desc }, i) => (
                    <Reveal
                        key={id}
                        delay={i * 100}
                        className="border border-white/10 p-6 md:p-9 rounded-[2px] transition-colors duration-250 hover:border-gold/40"
                    >
                        <div
                            className="font-serif text-[40px] md:text-[48px] font-bold leading-none mb-4 text-transparent [-webkit-text-stroke:1px_rgba(196,169,106,0.3)]"
                            aria-hidden="true"
                        >
                            {number}
                        </div>

                        <h3 className="font-serif text-[18px] md:text-[20px] font-semibold text-on-dark mb-2.5">
                            {title}
                        </h3>
